...
pub enum NetStreamType {
    ...
    F4v {
        ...
        /// ID of the video track being played (first one with a recognized codec, if any).
        video_track_id: Option<re_mp4::TrackId>,

        /// ID of the audio track being played (first one with a recognized codec, if any).
        audio_track_id: Option<re_mp4::TrackId>,
...
impl<'gc> NetStream<'gc> {
    ...
    pub fn execute_seek(self, context: &mut UpdateContext<'gc>, offset: f64) {
        ...
        if matches!(
            ...
            let (mp4_context, video_track_id, audio_track_id) = {
                ...
                match &*st {
                    Some(NetStreamType::F4v {
                        ...
                        audio_track_id,
                        ..
                    }) => (context.clone(), *video_track_id, *audio_track_id),
                    _ => unreachable!(),
                }
            };
    ...
    pub fn tick(self, context: &mut UpdateContext<'gc>, dt: FloatDuration) {
        ...
        }) = &mut *source.stream_type.borrow_mut()
        {
                ...
                match re_mp4::Mp4::read_bytes(&buffer[..complete_boxes_len]) {
                    Ok(mp4) => {
                        for (i, trak) in mp4.tracks().iter() {
                            match trak.kind {
                                Some(re_mp4::TrackKind::Video) if video_track_id.is_none() => {
                                ...
                                Some(re_mp4::TrackKind::Audio) if audio_track_id.is_none() => {
                                    *audio_track_id = Some(*i);
                                }
                                _ => {}
                            }
                        }
            ...
            let Some(media_context) = media_context else {
                return;
            };

            if let Some(vti) = *video_track_id {
                ...
                if video_stream.is_none() {
                    match context.video.register_video_stream(
                        ...
                        Ok(new_handle) => {
                            *video_stream = Some(new_handle);
                            // The track was only selected as `video_track_id`
                    ...
                    loop {
                        ...
                        *next_frame += 1;
                        ...
                        match context.video.decode_video_stream_frame(
                            ...
                            Err(e) => {
                                tracing::error!("Decoding video frame {} failed: {}", sample_id, e);
                            }
                        }
                    } // video loop
                }
            ...
            if let Some(ati) = *audio_track_id {
            ...
            let video_done = video_track_id.is_none() || video_exhausted;
            let audio_done = audio_track_id.is_none() || audio_exhausted;
