...
pub type PType = (
    EnumSet<PictureOption>,
    Option<(SourceFormat, PictureTypeCode)>,
);
...
///
/// A field is only listed in here if the H.263 spec mentions the requirement
/// that `UFEP` equal 001. Otherwise, the existence of a follower can be
/// determined by the set of `PictureOption`s returned in the `PlusPType`.
#[derive(EnumSetType)]
pub enum PlusPTypeFollower {
    HasCustomFormat,
    HasCustomClock,
...
const OPPTYPE_OPTIONS: EnumSet<PictureOption> = enum_set!(
...
fn decode_plusptype<R>(
    ...
    previous_picture_options: EnumSet<PictureOption>,
) -> Result<PlusPType>
where
    R: Read,
{
    reader.with_transaction(|reader| {
        ...
        let mut options = EnumSet::empty();
        let mut followers = EnumSet::empty();
...
type SorensonPType = (SourceFormat, PictureTypeCode, EnumSet<PictureOption>);
        ...
        let (mut format, picture_type, followers, has_plusptype, has_opptype) =
            match maybe_format_and_type {
                Some((format, picture_type)) => {
                    (Some(format), picture_type, EnumSet::empty(), false, false)
                }
                None => {
