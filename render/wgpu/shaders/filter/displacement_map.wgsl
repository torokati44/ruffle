...
fn get_component(map: vec4<f32>, component: u32) -> i32 {
    switch (component) {
        case 1u: {
            return i32(map.r * 255.0);
        }
        case 2u: {
            return i32(map.g * 255.0);
        }
        case 4u: {
            return i32(map.b * 255.0);
        }
        case 8u: {
            return i32(map.a * 255.0);
        }
        default: {
            return 0;
        }
...
fn displace_coordinates(original: vec2<i32>, map: vec4<f32>, components: vec2<u32>, scale: vec2<i32>) -> vec2<i32> {
    ...
    return vec2<i32>(
        original.x + (((get_component(map, components.x) - 128) * scale.x) / 256),
        original.y + (((get_component(map, components.y) - 128) * scale.y) / 256),
    );
}
...
fn main_fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var source_pos = vec2<i32>(
        i32(in.uv.x * filter_args.source_width),
        i32(in.uv.y * filter_args.source_height),
    );
    var map_uv = vec2<f32>(
        f32(source_pos.x - filter_args.offset_x) / filter_args.map_width,
        f32(source_pos.y - filter_args.offset_y) / filter_args.map_height,
    );
    ...
    let displaced = displace_coordinates(source_pos, map, components, vec2<i32>(filter_args.scale_x, filter_args.scale_y));
    var displaced_uv = vec2<f32>(
        f32(displaced.x) / filter_args.source_width,
        f32(displaced.y) / filter_args.source_height,
    );
